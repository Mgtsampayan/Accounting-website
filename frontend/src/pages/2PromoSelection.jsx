...
import Video2 from '../images/3.mp4';
import Video3 from '../images/4.mp4';
import { useRef, useEffect, memo } from "react";
import PropTypes from 'prop-types';

import BackgroundImage from '../images/bg_3.png';
...
const PromotionSelection = () => {
    const videos = [
        { src: Video1, autoPlay: true },
        { src: Video2, autoPlay: true },
        { src: Video3, autoPlay: true },
    ];
            ...
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 sm:gap-12 md:gap-16 lg:gap-32 relative z-10">
                <div className="w-full md:w-1/2 flex flex-col gap-6 sm:gap-8 md:gap-12 order-2 md:order-1">
                    {videos.map((video, index) => (
                        <div key={index} className={`flex justify-center ${index % 2 === 0 ? "md:justify-start" : "md:justify-end"}`}>
                            <div className="w-full sm:w-11/12 md:w-4/5">
                                <VideoPlayer videoSrc={video.src} autoPlay={video.autoPlay} />
                            </div>
                        </div>
                    ))}
                </div>
